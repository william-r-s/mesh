        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            with mtf.utils.outside_all_rewrites():
                ...
                if init_checkpoint:
                    ...
                    tf.logging.info(
                        "Variables in %s but not in graph:", init_checkpoint
                    )
                    tf.logging.info("\n".join(sorted(ckpt_vars - global_vars)))
                    tf.logging.info(
                        "Variables in graph but not in %s:", init_checkpoint
                    )
            ...
            def simple_metrics(xent, predictions, labels, weights):
                ...
                token_correct = (
                    tf.cast(tf.equal(predictions, labels), tf.float32) * weights
                )
                sequence_correct = tf.cast(
                    tf.equal(
                        tf.reduce_sum(token_correct, -1), tf.reduce_sum(weights, -1)
                    ),
                    tf.float32,
                )
