...
def get_variable_dtype(
    master_dtype=tf.bfloat16, slice_dtype=tf.float32, activation_dtype=tf.float32
):
...
@gin.configurable
def tpu_mesh_shape(
    tpu_topology=gin.REQUIRED, model_parallelism=gin.REQUIRED, ensemble_parallelism=None
):
...
def tpu_estimator_model_fn(
  ...
  """
    ...
    if use_tpu and "context" in params:
        ...
        physical_shape = list(params["context"].device_assignment.topology.mesh_shape)
    ...
    if outer_batch_size and mode not in [tf.estimator.ModeKeys.PREDICT, "score"]:
    ...
    ensemble_dims = (
        [mtf.Dimension("ensemble", ensemble_inputs)] if ensemble_inputs else []
    )
    ...
    for key, x in features.items():
        ...
        feature_shape = mtf.Shape(ensemble_dims + batch_dims + [length_dim])
        x = tf.cast(features[key], tf.int32)
        x = tf.reshape(x, feature_shape.to_integer_list)
        if not use_tpu:
            tf.logging.info("feature %s : %s" % (key, x))
            x = tf.Print(x, [x], "import feature %s" % key, summarize=1000, first_n=10)
    ...
    def _verify_feature_exists(feature_name, should_exist):
        if should_exist != (feature_name in mtf_features):
            message = "mode=%s model_type=%s should%s have feature %s" % (
                mode,
                model_type,
    ...
    if mode == "score":
        ...
        if isinstance(transformer_model, transformer.Unitransformer):
            ...
            inputs = mtf.shift(
                mtf_features["targets"], offset=1, dim=length_dim, wrap=False
            )
        elif isinstance(
            transformer_model, (transformer.Bitransformer, transformer.StudentTeacher)
        ):
        ...
        cross_entropy *= mtf.cast(mtf.not_equal(targets, 0), cross_entropy.dtype)
        if mode == "delimited_lm":
            cross_entropy *= mtf.cast(
                mtf.logical_not(transformer.delimited_lm_inputs_mask(targets)),
                cross_entropy.dtype,
            )
        scores = -mtf.reduce_sum(cross_entropy, reduced_dim=length_dim)
        scores = mtf.anonymize(scores)
        lowering = mtf.Lowering(graph, {mesh: mesh_impl}, autostack=autostack)
        predictions = {"scores": lowering.export_to_tf_tensor(scores)}
        ...
        if predict_fn:
            ...
            inputs = mtf.pad(inputs, [0, sequence_length["targets"]], length_dim.name)
    ...
    def logits_and_loss(mtf_features, num_microbatches=1):
      ...
      """
        ...
        if isinstance(transformer_model, transformer.Unitransformer):
            ...
            isinstance(transformer_model, transformer.Bitransformer)
            or model_type == "bi_student_teacher"
        ):
            position_kwargs = dict(
                encoder_sequence_id=mtf_features.get("inputs_segmentation", None),
                decoder_sequence_id=mtf_features.get("targets_segmentation", None),
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        trainable_vars = [v for v in graph.trainable_variables if variable_filter_fn(v)]
        trainable_var_grads = [
            g
            for g, v in zip(var_grads, graph.trainable_variables)
        ...
        if len(trainable_vars) != len(graph.trainable_variables):
            ...
            tf.logging.info(
                [v.name for v in graph.trainable_variables if not variable_filter_fn(v)]
            )
        ...
        lowering = mtf.Lowering(
            graph, {mesh: mesh_impl}, autostack=autostack, log_file=model_info_file
        )
        ...
        with mtf.utils.outside_all_rewrites():
            ...
            if use_tpu:
                return tpu_estimator.TPUEstimatorSpec(
                    ...
                    train_op=train_op,
                    host_call=host_call,
                    training_hooks=[restore_hook, saver_hook, gin_config_saver_hook],
                )
            else:
        ...
        def simple_metrics(xent, predictions, labels, weights):
            ...
            token_correct = tf.cast(tf.equal(predictions, labels), tf.float32) * weights
            sequence_correct = tf.cast(
                tf.equal(tf.reduce_sum(token_correct, -1), tf.reduce_sum(weights, -1)),
                tf.float32,
            )
            sequence_weights = tf.cast(
                tf.not_equal(tf.reduce_sum(weights, -1), 0), tf.float32
            )
            ...
            return {
                ...
                "max_targets_length": metric_max(
                    tf.reduce_sum(weights, axis=-1), name="max_targets_length"
                ),
            }
...
def _dynamic_text2self(mtf_features):
  ...
  Returns:
    a feature dictionary containing
      "targets", "targets_segmentation", "targets_position"
  """
    tf.logging.info("_dynamic_text2self: Converting text2text problem to text2self")
    inputs = mtf_features["inputs"]
    targets = mtf_features["targets"]
    ...
    new_length_range = mtf.range(targets.mesh, new_length_dim, dtype=tf.int32)
    ...
    inputs_permutation = mtf.to_int32(mtf.equal(new_length_range, inputs_destination))
    targets_destination = targets_position + mtf.gather(
        combined_segment_length_cumulative, targets_segmentation, segments_dim
    )
    targets_permutation = mtf.to_int32(mtf.equal(new_length_range, targets_destination))
    # map from the old length dimension to the new length dimension
    def _convert(t, perm):
        return mtf.rename_dimension(
            mtf.einsum(
                [t, perm], output_shape=inputs.shape.dims[:-1] + [new_length_dim]
            ),
            "new_length",
            "length",
        )

    targets = _convert(inputs, inputs_permutation) + _convert(
        targets, targets_permutation
    )
    targets_segmentation = _convert(inputs_segmentation, inputs_permutation) + _convert(
        targets_segmentation, targets_permutation
    )
    targets_position = _convert(inputs_position, inputs_permutation) + _convert(
        targets_position, targets_permutation
    )
    mtf_features = {
        "targets": targets,
...
def encode_inputs(
    inputs, vocabulary, model_type, batch_size, sequence_length, eos_id=1
):
...
@gin.configurable
def decode(estimator, input_fn, vocabulary, checkpoint_path=None):
...
@gin.configurable
def compute_log_likelihoods(estimator, input_fn, checkpoint_path=None):
    """Decode from an input_fn.
...
def clean_decodes(ids, eos_id=1, pad_id=0, length_axis=-1):
  ...
  """
    eos_and_after = tf.cumsum(
        tf.cast(tf.equal(ids, eos_id), tf.int32), exclusive=True, axis=length_axis
    )
    valid_ids = tf.equal(eos_and_after, 0)
    return tf.where_v2(valid_ids, ids, pad_id)


def _score_with_estimator(
    estimator,
    input_fn,
...
def score_from_strings(
  ...
  """
    ...
    if has_inputs:
        if len(inputs) < len(targets):
            ...
            if len(targets) % len(inputs):
                raise ValueError(
                    "len(inputs) must divide len(targets), got %d and %d"
    ...
    def input_fn(params):
        del params
        m = (
            {"inputs": all_input_ids, "targets": all_target_ids}
            if has_inputs
...
def score_from_dataset(
  ...
  """
    ...
    def input_fn(params):
        ...
        # Pad the final batch.
        dataset = transformer_dataset.trim_and_pad_dataset(dataset, length=batch_size)
        dataset = dataset.prefetch(tf.data.experimental.AUTOTUNE)
        return dataset
    ...
    return _score_with_estimator(
        estimator, input_fn, eval_checkpoint_step, model_dir, scores_filename, None
    )
        ...
        for eval_dataset in eval_datasets:
            if eval_dataset.metric_fns:
                ...
                inputs_filename = os.path.join(
                    eval_summary_dir, "{}_inputs".format(eval_dataset.name)
                )
...
def export_model(
  ...
  """
    ...
    """
        inputs = tf.placeholder(dtype=tf.string, shape=[None], name="inputs")
        ...
        dataset = transformer_dataset.pack_or_pad(
            dataset=dataset, length=sequence_length, pack=False, feature_keys=["inputs"]
        )
...
def compute_batch_size(sequence_length, mesh_shape, layout_rules, method_and_value):
  ...
  """
    ...
    num_replicas = mtf.tensor_dim_to_mesh_dim_size(
...
@gin.configurable
def serialize_num_microbatches(
    batch_dim,
    sequence_length,
    mesh_shape,
    layout_rules,
...
@gin.configurable
def auto_train_steps(batch_size, sequence_length, train_tokens=2 ** 36):
...
def get_checkpoint_iterator(checkpoint_step, model_dir, skip_until=0):
  ...
  """
    ...
    if checkpoint_step == "all":
        ...
        return filter(
            _filter_fn, [_get_checkpoint_path(s) for s in sorted(list(ckpt_steps))]
        )
...
def run(
  ...
  """
    if isinstance(sequence_length, int):
        sequence_length = {"inputs": sequence_length, "targets": sequence_length}
    ...
    tf.logging.info("Building TPUConfig with tpu_job_name={}".format(tpu_job_name))
    ...
    if mode == "train":
        ...
        if export_checkpoint_step:
            checkpoint_path = get_checkpoint_iterator(export_checkpoint_step, model_dir)
        ...
        raise ValueError(
            "unknown mode %s - must be train/perplexity_eval/eval/infer/export" % mode
        )
